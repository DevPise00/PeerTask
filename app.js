...
const Store = (() => {
  ...
  const seed = () => {
    ...
    save(LS_KEYS.stats, { skillsOffered: 0, servicesBooked: 0, trustScore: 0, reviews: 0 });
  ...
  return { state, init, persist, recalcStats };
})();
...
const Tabs = (() => {
  ...
  const switchTo = (name) => {
    ...
    // Scroll the active panel into view on small screens
...
const App = (() => {
  const init = () => {
    ...
    // Intercept in-page anchor links (header nav + hero buttons) so they
    // switch tabs instead of just changing the hash.
    document.addEventListener('click', (e) => {
      const a = e.target.closest('a[href^="#"]');
      if (!a) return;
      const name = a.getAttribute('href').replace('#', '');
      if (['skills', 'hire', 'dashboard'].includes(name)) {
        e.preventDefault();
        Tabs.switchTo(name);
      }
    });
